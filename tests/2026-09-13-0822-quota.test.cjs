...
test('all storage error shapes fail closed and handler emits 503',async()=>{
  ...
  for(const get of bad){const api=setup(async()=>get());assert.equal(await api.ratelimit('192.0.2.1',1,false),'unavailable');const res=response();await api.handler(req(),res);assert.equal(res.statusCode,503);}
...
test('hung quota service is aborted at three seconds',async()=>{
  ...
  try {assert.equal(await api.ratelimit('192.0.2.1',1,false),'unavailable');assert.ok(Date.now()-start>=2800);assert.ok(Date.now()-start<4500);}finally{clearTimeout(keepAlive);}
});
test('page spoofing does not grant classroom metering; allowlisted platform IP does',async()=>{
  async function run(page,ip,forwarded){
    let units;const api=setup(async(url,options)=>{assert.equal(url,'https://quota.invalid/pipeline');units=JSON.parse(options.body)[0][7];return Response.json(pipeline(4));},{MIKA_COURSE_IPS:'192.0.2.7'});
    const request=req(page,ip);if(forwarded)request.headers['x-forwarded-for']=forwarded;
    const res=response();await api.handler(request,res);assert.equal(res.statusCode,429);return units;
  }
  assert.equal(await run('/courses/test','192.0.2.1'),5);
  assert.equal(await run('/courses/test','192.0.2.7'),1);
  assert.equal(await run('/articles/test','192.0.2.7'),5);
  assert.equal(await run('/courses/test','192.0.2.1','192.0.2.7'),5);
  assert.equal(await run('/courses/test','192.0.2.7, 192.0.2.9'),1);
});
test('all four atomic quota refusal codes map to the correct client reason',async()=>{
 for(const [code,want] of [[0,'ok'],[1,'minute'],[2,'ip-day'],[3,'site-day'],[4,'site-month']]){
   ...
   assert.equal(await api.ratelimit('192.0.2.1',1,false),want);
 }
});
test('zero limits are preserved and invalid limits fail closed before storage',async()=>{
 ...
 assert.equal(await api.ratelimit('192.0.2.1',1,false),'site-day');
 const bad=setup(async()=>{throw Error('must not fetch');},{MIKA_DAILY_LIMIT:'invalid'});
 assert.equal(await bad.ratelimit('192.0.2.1',1,false),'unavailable');assert.equal(calls,1);
});
...
test('classroom IPv6 canonical addresses and bounded CIDRs match, outside ranges do not',async()=>{
  async function units(ip,allow){let units;const api=setup(async(url,options)=>{units=JSON.parse(options.body)[0][7];return Response.json(pipeline(4));},{MIKA_COURSE_IPS:allow});await api.handler(req('/courses/test',ip),response());return units;}
  assert.equal(await units('2001:db8::1','2001:0db8:0:0:0:0:0:1'),1);
  assert.equal(await units('2001:db8:7::999','2001:db8:7::/64'),1);
  assert.equal(await units('2001:db8:8::999','2001:db8:7::/64'),5);
  assert.equal(await units('192.0.2.7','192.0.2.0/24'),1);
  assert.equal(await units('192.0.3.7','192.0.2.0/24'),5);
  assert.equal(await units('192.0.2.7','0.0.0.0/0'),5);
});
...
test('IPv6 address rotation shares network bucket, including equivalent and mapped addresses',async()=>{
 ...
 await api.ratelimit('2001:db8:1:2::1',1,false);await api.ratelimit('2001:db8:1:2::ff',1,false);assert.equal(keys[0],keys[1]);
});
